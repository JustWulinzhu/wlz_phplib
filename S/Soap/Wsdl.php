...
class Wsdl {
    ...
    public function getWSDL() {
        ...
        $class_info = explode("\\", $this->class_name);
        $file_name = end($class_info) . ".wsdl";
        $file_path = '/www/image/' . $file_name;

        if (! file_exists($file_path)) {
            $fso = fopen($file_path, "w");
            fwrite($fso, sprintf('%s%s%s%s%s%s', $headerWSDL, $portTypeWSDL, $bindingWSDL, $serviceWSDL, $messageWSDL, '</definitions>'));
        }

        return IMAGE_SERVER_DOMAIN . DIRECTORY_SEPARATOR . $file_name;
    }
